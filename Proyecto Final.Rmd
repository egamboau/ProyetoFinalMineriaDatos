...
Es posible ver que, para este modelo en particular, se indicara que, para todas las posibles entradas, el valor será de cero, lo que quiere decir que ningún cliente comprará un seguro.
Ahora se intenta usar las variables del último modelo de regresión lineal.
```{r, tidy=TRUE}
...
A fin de verificar la creación del modelo, se procede a imprimir uno de los árboles creados en los bosques
```{r modelo-bosque-aleatorio-imprimir}
getTree(modelo.bosque, 3)
getTree(modelo2.bosque, 3)
```
...
regresion.prueba$proba_si <- predict(modelo.logistica, newdata = datos.prueba, type = "response")
table(regresion.prueba$CARAVAN, regresion.prueba$proba_si >= 0.5)

datos.entrenamiento$MRELSA
datos.prueba$MRELSA
...
95/(95+4)
  #Exactitud
(95+1)/nrow(regresion.test)
#ROC
regresion.roc <- prediction(regresion.test$yes_prob, regresion.test$family.history)
...
La exactitud del modelo es de 92%, especificidad de 94%. Sin embargo, su sensibilidad es tan solo de 12%.
...
+ Los modelos no parecen ser adecuados para contestar la pregunta inicial.