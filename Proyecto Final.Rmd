...
A fin de poder conocer el mercado de los seguro para caravanas, un vehículo que está equipado para vivir en él, se realizaron muestras de diversas variables del contexto de futuros clientes, a fin de poder conocer si, dado un cierto grupo de características, la persona compraría un seguro o no. 
...
1. Las siguientes columnas comparten su dominio, los valores que solo pertenecen a una sola columna están especificados en la lista anterior:
    ...
    1. Las columnas de la 44 a la 64, todas son valores monetarios. Sus posibles valores son:
        ...
        8. 10.000 a 19.999
        9. 20.000 o mayor
    1. Las columnas de la 45  a la 85, son numéricas, con rango de 1 a 12
...
barplot(table(compraronSeguros$MOSTYPE),
        main = "Distribución de tipos de clientes que compraron seguro",
...
barplot(table(datos.entrenamiento$MOSTYPE),
        main = "Distribución de tipos de clientes en el conjunto de datos",
        xlab = 'Tipo de cliente',
        ylab = 'cantidad de clientes')
```
...
A fin de completar el análisis, se procede a comparar las personas que, esta vez, no compraron seguro, a fin de conocer la proporción entre los tipos de cliente
...
barplot(table(noCompraronSeguros$MOSTYPE),
        ...
        ylab = 'cantidad de clientes')
```
Aquí es claro ver que la gran mayoría de los clientes que no compraron seguro fueron los de baja clase, lo que puede indicar que la clase es un factor determinante en caso de ser necesaria una predicción.
A continuación se analiza la variable de respuesta por edad promedio para ver si existen relaciones entre ambas variables.
```{r, tidy=TRUE}
library("lattice")
...
Es posible ver que, para este modelo particular, se indicara que, para todas las posibles entradas, el valor será de cero, lo que quiere decir que ningún cliente comprará un seguro.
...
A fin de verificar la creación del modelo, se procede a imprimir uno de los árboles creados en el bosque
...
Es importante notar que, en este caso, los datos son mucho mas grandes y con variables numerosas, lo que complcia un poco la creacion de los modelo. Se debe de realizar un estudio mas grande de las variables que se encuentran en el dominio de las observaciones, a fin de poder obtener de manera fiel una prediccion, con cualquier modelo. En el caso de este avance, mas exploracion es necesaria a fin de poder discernir las variables a usar, ademas de un refinamiento en los modelos que se utilizaron para el desarrollo de este documento.
## Evaluación
